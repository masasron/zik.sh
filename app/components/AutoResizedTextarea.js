...
function AutoResizedTextarea(props) {
    ...
    return <>
        ...
        />
        <span className={`material-symbols-outlined ${value.trim().length > 0 ? "active" : ""} ${loading ? "loading" : ""}`} onClick={(e) => !loading ? handleSubmit(e) : props.onStop()}>{!loading ? "send" : "stop"}</span>
